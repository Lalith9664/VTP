...
export interface TrendChartProps {
  /** Array from GET /api/dashboard/insights → trending_skills or legacy TrendingSkill[] */
  data: TrendSkillItem[];
...
const CustomTooltip = ({ active, payload }: any) => {
  ...
  const name = dataItem.skill_name || dataItem.name || "Skill";
  const frequency = dataItem.frequency || dataItem.popularity || 0;
  return (
    <div
      style={{
...
export function TrendChart({ data, title = "📊 Trending Skills Today" }: TrendChartProps) {
  ...
  // Normalize data for Recharts to support both legacy and backend fields
  const normalizedData = data.map((item) => ({
    skill_name: item.skill_name || item.name || "",
    frequency: item.frequency || item.popularity || 0,
  }));
